...
function CategoryCard({ setOpenForm, hasVoted, category, setCategory, id, categoryLength, totalVotes, mentorPerCategory }) {
    ...
    console.log(mentorPerCategory)
    return <>
        <CategoryName categoryID={categoryID} setCategoryID={setCategoryID} categoryName={category.name} categoryLength={categoryLength} />
        <div className="h-[300px] w-[80%] mentor-poll flex gap-3 items-end mb-4 shadow-inner  shadow-[#d0a4511f] p-2 md:p-4 rounded-md text-center">
            {mentorPerCategory && mentorPerCategory.map((mentor)=>{
            ...
            <MentorProfile height='160px' mentorName={'haha'}/>
        </div>
